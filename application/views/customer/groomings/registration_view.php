            ...
            <div id="content">
                ...
                <div class="container-fluid d-flex flex-column align-items-center">
                    ...
                    <div class="mb-4 text-center">
                        <h1 class="page-heading h3 text-gray-800">
                            <?= $page_title; ?>
                        </h1>
                    </div>

                    <div class="card border border-primary border-3 shadow p-3 mb-5 rounded-5"
                                    ...
                                    <form action="<?= base_url("grooming/register") ?>" method="post">
                                        ...
                                        <div class="form-group mt-5">
                                            ...
                                            <input type="text" id="customer_name" name="customer_name"
                                                class="form-control <?= form_error('customer_name') ? 'is-invalid' : ''; ?>"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" id="customer_phone" name="customer_phone"
                                                class="form-control <?= form_error('customer_phone') ? 'is-invalid' : ''; ?>"
                                                value="<?= $this->session->userdata("phone"); ?>">
                                            <?= form_error('customer_phone', '<div class="invalid-feedback font-weight-bold pl-1">', '</div>') ?>
                                        </div>
                                        <div class="form-group">
                                            <label for="customer_address"><i class="fas fa-map-marker-alt"></i> Alamat
                                                Customer</label>
                                            <textarea name="customer_address" id="customer_address" rows="3"
                                                class="form-control <?= form_error('customer_address') ? 'is-invalid' : ''; ?>"><?= $this->session->userdata("address"); ?></textarea>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="pet_type" id="pet_type"
                                                class="form-control <?= form_error('pet_type') ? 'is-invalid' : ''; ?>">
                                        ...
                                        <div class="form-group d-none" id="catPackage">
                                            <label for="package_id"><i class="fas fa-box"></i> Paket Boarding Service
                                                Kucing</label>
                                            <select name="package_id" id="package_id" class="form-control">
                                                <option value="" disabled selected>--Pilih Paket Pet Boarding Service--
                                                </option>
                                                <?php foreach ($packages as $package): ?>
                                                    <option value="<?= $package["package_id"] ?>">
                                                        <?= $package["name"] ?> | IDR.
                                                        <?= number_format($package["cost_for_cat"]) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                        <div class="form-group d-none" id="dogPackage">
                                            ...
                                            <select name="package_id" id="package_id" class="form-control">
                                                ...
                                                <?php foreach ($packages as $package): ?>
                                                    <option value="<?= $package["package_id"] ?>">
                                                        <?= $package["name"] ?> | IDR.
                                                        <?= number_format($package["cost_for_dog"]) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label for="date_created"><i class="fas fa-calendar-alt"></i> Tanggal
                                                Check-in</label>
                                            <input type="date" id="date_created" name="date_created"
                                                class="form-control <?= form_error('date_created') ? 'is-invalid' : ''; ?>"
                                                value="<?= set_value('date_created'); ?>">
                                            <?= form_error('date_created', '<div class="invalid-feedback font-weight-bold pl-1">', '</div>') ?>
                                        </div>
                                        <div class="form-group">
                                            <label for="date_finished"><i class="fas fa-calendar-alt"></i> Tanggal
                                                Check-out</label>
                                            <input type="date" id="date_finished" name="date_finished"
                                                class="form-control <?= form_error('date_finished') ? 'is-invalid' : ''; ?>"
                                                value="<?= set_value('date_finished'); ?>">
                                            <?= form_error('date_finished', '<div class="invalid-feedback font-weight-bold pl-1">', '</div>') ?>
                                        </div>
                                        <div class="form-group">
                                            <label for="notes"><i class="fas fa-sticky-note"></i> Catatan Customer
                                                (Opsional)</label>
                                        ...
                                        <div class="form-action d-flex justify-content-center p-3">
                                            <button type="submit" class="btn btn-primary mr-4 p-2"><i
                                                    class="fas fa-check"></i> Register Pet Boarding Service</button>
                                            <a href="<?= base_url('landing'); ?>" class="btn btn-warning p-2"><i
                                                    class="fas fa-times"></i> Batalkan</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
